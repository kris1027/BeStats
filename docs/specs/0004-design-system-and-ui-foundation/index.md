...
- [x] Assumption confirmed in build task 5: the installed `@base-ui/react` Dialog supplies the focus trap, scroll lock, inert background and focus return AC-15 requires. `mobile-menu-sheet.test.tsx` asserts focus entry, the inert background, Escape and focus return; Tab containment and scroll locking are browser only, so they were confirmed by hand on `/showcase`.
