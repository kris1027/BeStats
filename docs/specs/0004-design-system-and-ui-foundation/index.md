...
- [ ] `AGENTS.md` does not yet record the UI conventions this spec establishes (the plate rule, the token vocabulary, the server by default policy). They are project wide and belong in root `AGENTS.md` once built, or in a nested `components/AGENTS.md` if they grow.
- [ ] Two assumptions carried by this spec, both to confirm during the build rather than resolve now. First, that the installed `@base-ui/react` Dialog supplies the focus trap, scroll lock, inert background and focus return AC-15 requires; build task 5 checks it. Second, that `usePathname` in the segmented control does not push the shell out of the static shell under `cacheComponents`; build task 3 checks it.
