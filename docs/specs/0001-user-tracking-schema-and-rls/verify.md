...
- [ ] `pnpm exec supabase db reset` → applies the one migration and the seed with no error → AC-1
- [ ] `pnpm exec supabase db reset && pnpm exec supabase db dump --local -f /tmp/a.sql && pnpm exec supabase db reset && pnpm exec supabase db dump --local -f /tmp/b.sql && diff /tmp/a.sql /tmp/b.sql` → no output, the two dumps are identical → AC-1
- [ ] `pnpm test:db` → all files pass, 69 assertions, `Result: PASS` → AC-2 to AC-13
- [ ] `pnpm exec supabase db advisors --local` → `No issues found`. Treat as a net, not a proof: the advisors check that row level security is enabled, not that it is forced → AC-2
- [ ] `pnpm db:types:check` → `matches the database schema`. Then change any column in `supabase/schemas/02-tables.sql`, regenerate the migration, reset, and rerun → it fails and names the drift → AC-14
- [ ] `pnpm typecheck` → passes → AC-14
- [ ] `pnpm lint` → passes → AC-14
- [ ] Stop the local stack, then `pnpm build` → succeeds with no database reachable → AC-14
- [ ] `git grep -n -iE "service_role|sb_secret" -- lib app scripts .env.example` → no match outside skill documentation → AC-15
- [ ] After `pnpm build`, `grep -r "service_role\|sb_secret" .next/static` → no match → AC-15
...
- [ ] Upsert an episode sending only `watched_at`, on an episode that already carries a rating → the rating survives. Send the full row instead and it does not: this is the partial write rule, and the most likely way to break AC-6 and AC-10 → AC-6, AC-10
- [ ] Upsert the same movie twice with different ratings → one row, carrying the later rating → AC-6
- [ ] Clear `watched_at` to null on a rated movie → the rating is untouched. Clear `rating` to null → the rest of the row is untouched → AC-10
- [ ] Insert a rating of 0, then 11, then -3 → each rejected with `23514` by the database itself, not by Zod → AC-7
- [ ] Insert a status of `binging`, then a `status_source` of `robot` → each rejected with `22P02` → AC-8
- [ ] Insert `season_number` 0 → accepted, so specials are storable. Insert -1, and `episode_number` 0 → each rejected → AC-9
- [ ] Insert an episode row for a show id with no `user_show_state` row → succeeds. Episode history must not depend on a status existing → AC-13
- [ ] `delete from auth.users where id = '22222222-2222-2222-2222-222222222222'` → no rows remain for that id in any of the three tables → AC-11
- [ ] As `anon` (`set local role anon`), select and insert on each table → permission error `42501`, not an empty result. An empty result would mean only row level security is stopping it → AC-4
