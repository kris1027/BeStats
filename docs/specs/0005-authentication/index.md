...
| navbar account slot | signed in or signed out | `getOptionalUser()` in the slot's server component, which uses `getClaims()`. A local check, so a session revoked elsewhere still reads as signed in until its access token expires, at most `jwt_expiry = 600` seconds (AC-8) |
